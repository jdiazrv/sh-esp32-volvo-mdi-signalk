...
2. Usar los botones al observar ON, inicio/fin de precalentamiento, inicio/fin de pitidos, pulsación de START, fin del giro de arranque, motor funcionando, pulsación de STOP y motor detenido. También están disponibles las observaciones separadas de luz de aceite y luz de carga, incluida «no clara» para no convertir una duda en apagada.
...
Al iniciar una observación se reinicia la línea base de los selectores propietarios. Durante aproximadamente 90 segundos alrededor de actividad del motor se conservan además muestras de RPM cada 250 ms y de temperatura/tensión aproximadamente cada segundo. Esto permite comparar el momento de la luz de aceite con el giro del motor sin llenar SPIFFS con cada variación estable.
...
La captura automática funciona sin pulsar «capturar». Conserva línea base y cambios relevantes, excluyendo el contador rodante conocido y cambios exclusivos de RPM, pero no aplica una espera de un segundo a las transiciones: una secuencia A→B→A queda registrada. Durante el contexto de arranque/parada añade muestras periódicas de RPM, temperatura y tensión. La versión publicada conserva para temperatura la línea base y cruces del umbral de 50 °C; no es un registro continuo de cada grado. Se usa una clave por PGN/origen y, para ciertos PGN propietarios, selector.
...
Columnas del CSV automático: `boot_id`, `utc`, `epoch_ms`, `uptime_ms`, `event`, `can_id_hex`, `pgn`, `pgn_hex`, `source_hex`, `length`, `raw_changed_mask`, `trigger_mask`, `context`, `session`, `xor_hex`, `data_hex`. El XOR compara con la trama anterior recibida de esa clave, no necesariamente con la anterior fila guardada.
